...
class TestPredictionsClient:
    ...
    def test_record_prediction(self, learn_client, clean_tables):
        ...
        fact_id = learn_client.atomic.predictions.record(
        ...
        fact = learn_client.atomic.predictions.get(fact_id)
    ...
    def test_empty_hypothesis_raises(self, learn_client, clean_tables):
        ...
        with pytest.raises(ValidationError, match="hypothesis cannot be empty"):
            learn_client.atomic.predictions.record(
                hypothesis="",
                confidence=0.5,
            )
    ...
    def test_resolve_correct(self, learn_client, clean_tables):
        ...
        result = learn_client.atomic.predictions.resolve(
            fact_id=fact_id,
            outcome="correct",
    ...
    def test_resolve_incorrect(self, learn_client, clean_tables):
        ...
        learn_client.atomic.predictions.resolve(
            fact_id=fact_id,
            outcome="incorrect",
    ...
    def test_resolve_already_resolved_raises(self, learn_client, clean_tables):
        ...
        learn_client.atomic.predictions.resolve(fact_id=fact_id, outcome="correct")

        # Second resolution should fail
        with pytest.raises(ValidationError, match="already resolved"):
            learn_client.atomic.predictions.resolve(fact_id=fact_id, outcome="incorrect")

    def test_list_pending(self, learn_client, clean_tables):
        """Test listing pending predictions."""
        learn_client.atomic.predictions.record(hypothesis="A", confidence=0.5)
        fact_id = learn_client.atomic.predictions.record(hypothesis="B", confidence=0.6)
        learn_client.atomic.predictions.record(hypothesis="C", confidence=0.7)

        # Resolve one
        learn_client.atomic.predictions.resolve(fact_id, "correct")

        pending = learn_client.atomic.predictions.list_pending()
    ...
    def test_get_calibration_stats(self, learn_client, clean_tables):
        ...
        for conf, outcome in [(0.3, "incorrect"), (0.7, "correct"), (0.9, "correct")]:
            ...
            learn_client.atomic.predictions.resolve(fact_id, outcome)

        stats = learn_client.atomic.predictions.get_calibration_stats()
