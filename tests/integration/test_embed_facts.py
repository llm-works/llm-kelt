...
class TestEmbedMissingFactsIntegration:
    ...
    async def test_embed_all_facts(self, logger, learn_client, mock_embedder, sample_facts):
        ...
        without_after = learn_client.embeddings.list_without_embeddings("test-model")
        assert len(without_after) == 0

        # Verify embeddings work for similarity search
        search_results = learn_client.embeddings.search_similar(
            query=[0.2, 0.3, 0.4],
    ...
    async def test_embed_skips_already_embedded(
        ...
        learn_client.embeddings.set_embedding(sample_facts[0], [0.1, 0.2, 0.3], "test-model")
        learn_client.embeddings.set_embedding(sample_facts[1], [0.4, 0.5, 0.6], "test-model")
    ...
    async def test_embed_different_model_embeds_all(
        ...
        for fact_id in sample_facts:
            learn_client.embeddings.set_embedding(fact_id, [0.1, 0.2, 0.3], "model-a")
    ...
    async def test_embed_respects_batch_size(
        ...
        result = await embed_missing_facts(
            logger, mock_embedder, learn_client.embeddings, batch_size=2
        )
